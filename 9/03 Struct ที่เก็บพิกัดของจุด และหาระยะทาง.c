...
ประกาศโครงสร้างข้อมูล struct Point เพื่อใช้เก็บตำแหน่งของจุดทั้ง 2 ที่อยู่บนแกน X, Y ซึ่งมีชนิดข้อมูลเป็น double
ที่ main() ประกาศตัวแปร array of struct ชื่อ points ให้มีชนิดข้อมูลเป็น struct Point เพื่อเก็บคู่อันดับของพิกัด X, Y จากผู้ใช้ จำนวน n คู่
เขียนคำสั่ง for เพื่ออ่านคู่อันดับของพิกัด X, Y จำนวน n คู่จากผู้ใช้ ตามตัวอย่างการรับค่าจาก standard input
เขียนคำสั่ง for โดยกำหนดให้ภายใน for loop ประกอบด้วย statements ต่อไปนี้
